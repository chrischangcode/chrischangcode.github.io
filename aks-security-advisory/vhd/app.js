...
let STATUS_HELP = {
  ...
  affected: "Vulnerable, with no fix AKS can ship yet. Comes from a vendor VEX / Ubuntu CVE Tracker assertion (a fixless CVE cannot be enumerated from OVAL alone), or from the binary scan overlay when a CVE survives to the newest scanned build. A scanned binary stays affected even if the scanner reports a fixed version, because that fix is in the bundled module and not in any artifact AKS ships — see evidence.module_fixed_version. The generated glossary records how large a share of the feed it is.",
...
// (single-sourced; the copy is asserted equal by tests/test_vocab.py). Used to
// render a friendly label for each evidence link on the advisory detail view.
let EVIDENCE_LABELS = {
  release_notes: "Release notes",
  release_notes_raw: "Release notes (raw)",
  release_notes_image_list: "Release notes image list",
  release_notes_latest: "Release notes (latest build)",
  release_notes_latest_raw: "Release notes (latest build, raw)",
  release_notes_latest_image_list: "Release notes image list (latest build)",
  advisory_feed: "Advisory feed",
  advisory: "Advisory",
  security_patch: "Security-patch version",
  path_package_index: "Path\u2192package index",
  scan: "Scanned artifact",
  scanner: "Scanner image",
  scan_pkg: "Scanned module",
  module_fixed_version: "Module fixed version (not a shipped fix)",
};
...
let FIELD_HELP = {
  ...
  release: "The AKS VHD lineage(s) — OS family + generation/SKU, e.g. 'AzureLinux-V3/gen2' — where this CVE was assessed. An advisory's packages[] row lists every lineage sharing that same assessment in its releases[], so match your SKU by membership, not equality. Use the OS/SKU filter to narrow the list.",
  ...
  first_fixed_before: "The oldest AKS VHD build in the scanned window, which already shipped this package at or above the fixed version. The fix therefore landed at or before this build; the exact build predates the window and is not identified. Present instead of 'first_fixed_build' -- the package is fixed, it was simply fixed before tracking began.",
...
const state = {
  ...
  // `pkgExact` is set only by a `?pkg=` deep link, where the name is taken
  // verbatim from feed data and a substring match would drag in unrelated
  // packages (a link for `ig` otherwise also matches grub-efi-amd64-signed,
  // libonig5, pigz and shim-signed). Typing in the box clears it, so manual
  // filtering stays incremental.
  filters: { q: "", release: "", status: "", severity: "", pkg: "", pkgExact: false },
...
function applyFilters() {
  const { q, release, status, severity, pkg, pkgExact } = state.filters;
  ...
  return state.entries.filter((e) => {
    ...
    if (pkgNeedle && !(e.packages || []).some((p) => {
      const lp = p.toLowerCase();
      return pkgExact ? lp === pkgNeedle : lp.includes(pkgNeedle);
    })) return false;
    return true;
  }).sort(compareCveDesc);
}
...
function renderControls() {
  ...
  pkgInput.addEventListener("input", () => {
    state.filters.pkg = pkgInput.value;
    state.filters.pkgExact = false;  // hand-typed filtering is incremental
    refilter();
  });
...
function resetFilters() {
  state.filters = { q: "", release: "", status: "", severity: "", pkg: "", pkgExact: false };
  state.page = 0;
...
function evidenceLinks(evidence) {
  ...
  const items = Object.entries(evidence)
    ...
    .map(([k, v]) => {
      ...
      return el("li", null, isExternal(v)
        ? [label + ": ", el("a", { href: v, target: "_blank", rel: "noopener" }, v)]
        : el("span", null, label + ": " + v));
    });
...
function packageCard(row) {
  ...
  kv(dl, "Fixed at or before", row.first_fixed_before && el("span", { class: "mono" }, row.first_fixed_before), "first_fixed_before");
  ...
  kv(dl, "Retained vulnerable version",
     ...
     "retained_vulnerable_version");
  // One row now covers every lineage sharing this assessment, so each lineage
  // gets its OWN paths link. A single ?os= would be wrong for ~30% of rows:
  // jammy and noble lineages merge into one row but have different installed
  // paths, so one link could not be right for both.
  // Tolerate a v1 (schema 1.0) feed: pages.yml sources the feed artifact from a
  // *separate* build-advisories run, so a manual deploy -- or a build that was
  // already in flight when this UI shipped -- can pair v2 site code with a v1
  // document that still carries the singular `release`. Without this the
  // Lineages row silently vanishes and the paths link loses its ?os=.
  const rels = row.releases || (row.release ? [row.release] : []);
  const relLinks = rels.map((rel, i) => [
    i ? ", " : "",
    el("a", {
      href: "#/paths?pkg=" + encodeURIComponent(row.package) +
            "&os=" + encodeURIComponent(rel),
      class: "mono rel-link",
      title: "Installed paths owned by " + row.package + " on " + rel,
    }, rel),
  ]).flat();
  if (relLinks.length) kv(dl, "Lineages", el("span", null, relLinks), "release");

  const ev = evidenceLinks(row.evidence);
  const card = el("div", { class: "card" }, [
    el("h3", null, [
      el("a", {
        // Deterministic and always one of THIS row's lineages, so the paths
        // page never opens on an unrelated default. The per-lineage links
        // above remain the precise entry points.
        href: "#/paths?pkg=" + encodeURIComponent(row.package) +
              (rels.length ? "&os=" + encodeURIComponent(rels[0]) : ""),
        class: "mono pkg-link",
  ...
  const additiveCard = el("div", { class: "card" }, [
    ...
    (add.items && add.items.length)
      ? el("ul", { class: "additive-items" }, add.items.map((i) => {
          ...
          // stale `package` left over from its covered_by_package days must not
          // be offered here -- it links to a package no advisory names.
          if (cov === "covered_by_scan") {
            kids.push(el("span", { class: "note" },
              advisoryPackageNames().has(String(i.name || "").toLowerCase())
                ? [" \u2192 ", el("a", { href: "#/?pkg=" + encodeURIComponent(i.name) },
                    "assessed by scan")]
                : " \u2192 scanned, no findings"));
          } else if (i.package) {
            kids.push(el("span", { class: "note" }, [
...
// "this link goes nowhere" apart from "this component was scanned and is clean",
// which are otherwise indistinguishable to a reader. Memoized on the entries
// array identity so a feed reload rebuilds it rather than serving a stale set.
let _pkgNameSet = { src: null, set: null };
function advisoryPackageNames() {
  const entries = state.entries || [];
  if (_pkgNameSet.src === entries) return _pkgNameSet.set;
  const set = new Set();
  for (const e of entries) {
    for (const p of e.packages || []) set.add(String(p).toLowerCase());
  }
...
function assessedCell(i) {
  ...
  // Checked BEFORE `i.package`: a scan-covered component is assessed by its own
  // scan-derived rows, which are keyed on the component name. `package` names the
  // distro package the component was originally resolved from, and it matches no
  // advisory once the scan takes over -- so a feed that still carries it (an
  // older artifact, since pages.yml can deploy a feed from a previous build)
  // must not win here and send the reader to an empty result.
  if (coverage === "covered_by_scan") {
    if (!advisoryPackageNames().has(String(i.name).toLowerCase())) {
      // Scanned, and nothing survived -- a real result, not a broken link.
      return el("span", { class: "note scan-clean" }, "scanned \u2014 no findings");
    }
  ...
  const view = el("div", { class: "help" }, [
    ...
    (() => {
      ...
      const order = ["cve", "severity", "status", "release", "updated",
        ...
        "first_fixed_build", "first_fixed_before", "latest_build", "advisory_id", "evidence",
        "references", "additive"];
    ...
    el("ul", { class: "bullets" }, [
      el("li", null, [el("strong", null, "Additive surface only partly assessed. "),
        FIELD_HELP.additive]),
      el("li", null, [el("strong", null, "Distro 'affected' comes only from vendor verdicts. "),
        "A fixless CVE can't be enumerated from OVAL alone, so for distro-packaged rows 'affected' is surfaced only where the vendor VEX / Ubuntu CVE Tracker asserts it; otherwise the feed reports fixed / fix_available_upstream / under_investigation. Scanned AKS-built binaries are different: they are reported 'affected' whenever the CVE survives to the newest scanned build, including when the scanner reports the bundled module as fixed \u2014 a module fix is not a shippable artifact. See the measured share beside each status above for how much of this build it is."]),
    ...
    el("dl", { class: "kv help-kv" }, [
      ...
      el("dd", null, [el("span", { class: "mono" },
        "{ schema_version, id, severity, headline_status, headline_status_note, updated, description?, references[], packages[], assessed_releases?, assessment_scope, additive }"),
        " \u2014 each packages[] row: ",
        el("span", { class: "mono" },
          "{ package, release, status, severity, advisory_id?, fixed_version?, upstream_fixed_version?, first_fixed_build?, first_fixed_before?, latest_build?, justification?, statement?, evidence? }"),
        "."]),
...
async function route() {
  ...
  if (/^help$/i.test(path)) {
    ...
    state.filters.pkgExact = !!state.filters.pkg;
    state.page = 0;
